	...
	for {
		...
		case err := <-chErr:
			if err != nil {
				...
				cm.reconnect()
				return // Exit after reconnect attempt, new monitor will be started
			}
		case <-cm.ctx.Done():
			return
		}
...
func (cm *ChannelManager) reconnect() {
	...
	// Add delay before reconnect to prevent CPU spike
	time.Sleep(cm.retryInterval)
	...
	if err != nil {
		logger.Error.Printf("Failed to reconnect channel: %v\n", err)
	} else {
